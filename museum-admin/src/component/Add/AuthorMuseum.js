...
const Sign =  React.memo(
    (props) =>{
        ...
        let [yearsOfLife, setYearsOfLife] = useState(selected!==-1?data[selected][1]:'');
        let handleYearsOfLife =  (event) => {
            setYearsOfLife(event.target.value)
        };
        let [biography_ru, setBiography_ru] = useState(selected!==-1?data[selected][2]:'');
        let handleBiography_ru =  (event) => {
            setBiography_ru(event.target.value)
        };
        let [biography_kg, setBiography_kg] = useState(selected!==-1?data[selected][3]:'');
        let handleBiography_kg =  (event) => {
            setBiography_kg(event.target.value)
        };
        let [biography_eng, setBiography_eng] = useState(selected!==-1?data[selected][4]:'');
            ...
            <div>
                ...
                <div>
                    <Button variant='contained' color='primary' onClick={()=>{
                        ...
                        if(selected===-1)
                            addData({search: search, sort: sort, page: page,  name: 'Автор произведения', data: {name: name, yearsOfLife: yearsOfLife, biography_ru: biography_ru, biography_kg: biography_kg, biography_eng: biography_eng}});
                        else
                            setData({id: data[selected][6], search: search, sort: sort, page: page, name: 'Автор произведения', data: {name: name, yearsOfLife: yearsOfLife, biography_ru: biography_ru, biography_kg: biography_kg, biography_eng: biography_eng}});
                        setSelected(-1)
